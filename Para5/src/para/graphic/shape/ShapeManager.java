...
public class ShapeManager {
  ...
  // }
  public synchronized boolean remove(int id) {
    Shape[] array_data = data.toArray(new Shape[data.size()]);
    IntStream.range(0, array_data.length).parallel().forEach(i -> {
      if (array_data[i].id == id) {
        Shape target_shape = array_data[i];
        array_data[i] = new Blank(id, (target_shape.top + target_shape.bottom) / 2,
            (target_shape.left + target_shape.right) / 2);
      }
      // removed = true;
    });
    data = new ArrayList<Shape>(Arrays.asList( array_data));
    return true;
  }
